...
function drawBuckets(context, startX, startY, bucket) {
    ...
    bucketImage.src = 'bucket.png'; // Replace 'bucket.png' with the actual path to your bucket image

    bucketImage.onload = function() {
        for (var i = 0; i < 10; i++) {
            // Draw the bucket image with increased width to 200 pixels
            context.drawImage(bucketImage, 80 * (i + 2) + startX + 70, 90 + startY, 55, 150);

            context.fillStyle = 'black';
            context.rect(80 * (i + 2) + startX + 70, 90 + startY, 200, 150);
            context.font = '18px Arial';
            context.fillText('b[' + i + ']', 80 * (i + 2) + startX + 78, 260 + startY);
            context.fillStyle = 'black';

            for (var j = 0; j < bucket[i].length; j++) {
                context.fillText(bucket[i][j], 80 * (i + 2) + startX + 80, j * 20 + 110 + startY);
            }
